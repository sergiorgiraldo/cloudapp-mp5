...
public final class KMeansMP {
	...
	final KMeansModel model;
        ...
        JavaRDD<Vector> parsedPoints = lines.map(new ParsePoint());
        JavaRDD<String> parsedTitles = lines.map(new ParseTitle());

	model = KMeans.train(parsedPoints.rdd(), k, iterations, runs, KMeans.RANDOM(), seed);

        results = parsedTitles.zip(parsedPoints).mapToPair(new ClusterCars(model)).groupByKey();
